...
class DersDetailsScreen extends ConsumerWidget {
  const DersDetailsScreen({
    super.key,
    required this.ders,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final favoriteMeals = ref.watch(favoriteDersProvider);
    ...
    return Scaffold(
      appBar: AppBar(title: Text(ders.title), actions: [
        IconButton(
          onPressed: () {
            final wasAdded = ref
                .read(favoriteDersProvider.notifier)
                .toggleDersFavoriteStatus(ders);
